...
def main():
    ...
    selection.add_argument(
        "--agent",
        help="额外机器人配置的 模块路径:类名；默认检查本目录 FR3",
    )
    selection.add_argument(
        "--model",
        choices=["official", "cad"],
        default="official",
        help="检查官方 FR3 Duo，或用 cad 对照随示例保留的旧版模型",
    )
    ...
    # 默认直接检查本目录已经填写好的 FR3 配置；--agent 仍可验收其他配置。
    class_name = "FR3DualArm" if args.model == "official" else "FR3DualArmCAD"
    spec = args.agent or f"{__package__}.robot:{class_name}"
    try:
