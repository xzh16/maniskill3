...
class PushCubeEnv(BaseEnv):
    ...
    **Success Conditions:**
    ...
    def _default_sensor_configs(self):
        # setting robot camera: registers one 128x128 camera looking at the robot, cube, and target
...
def make_tcp_action(
    ...
    """
    生成可供env.step()使用的绝对TCP动作。

    position: 机械臂根坐标系下的[x, y, z]，单位m
    orientation_rpy: [roll, pitch, yaw]，单位rad
    gripper: 通常1.0张开、-1.0闭合；None表示不设置
    """
    base_env = env.unwrapped
    controller = base_env.agent.controller

    if arm_key not in controller.controllers:
        raise KeyError(
            f"没有找到控制器'{arm_key}'，"
            f"可用控制器为{list(controller.controllers)}"
        )

    arm_controller = controller.controllers[arm_key]

    if getattr(arm_controller.config, "use_delta", True):
        raise ValueError(
            "当前不是绝对TCP控制器，请创建环境时使用"
            "control_mode='pd_ee_pose'"
        )

    arm_start, arm_end = controller.action_mapping[arm_key]

    if arm_end - arm_start != 6:
        raise ValueError(
            f"{arm_key}控制器需要{arm_end - arm_start}维动作，"
            "不是标准的6维TCP位姿"
        )
    ...
    tcp_pose = np.asarray(
        [*position, *orientation_rpy],
        dtype=np.float32,
    )
    action[..., arm_start:arm_end] = tcp_pose

    if gripper is not None and gripper_key in controller.action_mapping:
        gripper_start, gripper_end = controller.action_mapping[gripper_key]
        action[..., gripper_start:gripper_end] = gripper
...
def main():
    ...
    action = make_tcp_action(
        env,
        position=[0.45, 0.5, 0.25],
        orientation_rpy=[np.pi, 0.0, 0.0],
        gripper=1.0,
    )
